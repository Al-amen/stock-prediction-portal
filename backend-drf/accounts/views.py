...
class RegisterAPIView(generics.CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        user = serializer.save()  # inactive user by default

        # Generate verification token
        token = generate_email_verification_token(user)

        # Create verification link
        verify_link = f"http://127.0.0.1:8000{reverse('verify-email')}?token={token}"

        # Send email
        send_mail(
            subject="Verify your email",
            message=f"Hi {user.first_name or ''}, click to verify your account: {verify_link}",
...
class ResendVerificationAPIView(APIView):
    def post(self, request):
        ...
        try:
            ...
            token = generate_email_verification_token(user)
            verify_link = f"http://127.0.0.1:8000{reverse('resend-verify-email')}?token={token}"
            send_mail(
                "Verify your email",
            ...
            return Response({"detail": "No inactive account found with that email."}, status=404)
...
class PasswordResetRequestAPIView(APIView):
    ...
    def post(self, request):
        ...
        try:
            ...
            reset_link = f"http://127.0.0.1:8000/api/v1/user/reset-password-confirm/{uidb64}/{token}/"
